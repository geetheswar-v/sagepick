...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Starting popular categories sync...");

    const syncService = new CategorySyncService();
    const result = await syncService.syncPopular();

    if (result.success) {
      return NextResponse.json({
        success: true,
        message: "Popular categories sync completed successfully",
        jobId: result.jobId,
      });
    } else {
      return NextResponse.json(
        {
          success: false,
          message: "Popular categories sync failed",
          error: result.error,
          jobId: result.jobId,
        },
        { status: 500 }
